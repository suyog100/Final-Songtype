  ...
  devtools((set, get) => ({
    ...
    calculateResults: () => {
      ...
      if (get().gameMode === GAME_MODE.TIME) {
        ...
        // Calculate the wpm
        const averageWordLength =
          totalWords > 0 ? totalLetters / totalWords : 5; // Usa 5 como fallback si totalWords es 0
    ...
    appendCurrentWordAtTheEnd: () => {
      set((state) => {
        ...
        const currentWord = JSON.parse(
          JSON.stringify(newWords[state.wordIndex]),
        );
    ...
    setWordsFromLyrics: (lyrics) => {
      const wordsArray = lyrics.split(" "); // Split the lyrics into words
      const formattedWords = wordsArray.map((word) =>
        word.split("").map((letter, index) => ({
          letter,
          index,
          state: null, // Default state for each letter
        })),
      );

      if (formattedWords.length > 0 && formattedWords[0].length > 0) {
        formattedWords[0][0].state = "active"; // Set the first letter as active
      }

      set({
        words: formattedWords,
        wordsAsString: lyrics, // Store the original lyrics as a string
        wordIndex: 0,
        letterIndex: 0,
      });
    },
